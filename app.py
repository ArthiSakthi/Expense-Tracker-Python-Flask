...
app.config['SECRET_KEY'] = 'heyhi'
app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {'connect_args': {'timeout': 30}}  # Set timeout value
db = SQLAlchemy(app)
...
class User(UserMixin, db.Model):
    ...
    expenses = db.relationship('Expense', cascade='all, delete, delete-orphan',backref='user', lazy=True, passive_deletes=True)
...
def home():
    ...
    if request.method == 'POST':
        ...
        if not note:
            note = ''
        if not amount:
            flash('Please enter the amount paid in rupees', category='error')
        else:
    ...
    return render_template('home.html', user=current_user, expenses=expenses,amount_sort=amount_sort, category_sort=category_sort, date_sort=date_sort,get_total=get_total)

@app.route('/view-expense/<id>', methods=['GET'])
@login_required
def view_expense(id):
    ...
    return render_template('view_expense.html', user=current_user, expense=expense)
